...
import { IProjectRepository } from "@modules/project/domain/repositories/project-repository-interface.ts";
import { Project } from "@entities/project.ts";
import { SHIFT } from "@enums/shift-enum.ts";
import { NoItemsFoundError } from "@helpers/errors/domain-errors.ts";
import { ProjectJson } from "@jsons/project-json.ts";
...
export class ProjectRepositoryMock implements IProjectRepository {
    ...
    async getProjectsByRole(userId: number): Promise<Project[]> {
        const json = ProjectJson.projectJson.filter(
            (project) =>
                project.responsibles.some(
                    (responsible) => responsible.user_id === userId
                ) ||
                project.advisors.some(
                    (advisor) => advisor.user_id === userId
                ) ||
    ...
    async getProject(projectId: number): Promise<Project> {
        const json = ProjectJson.projectJson.find(
        ...
        ProjectJson.projectJson.forEach((project, index) => {
            if (project.project_id === projectId) {
                ...
                if (newQualification !== undefined)
                    projectToUpdate.qualification = newQualification;
                if (newCode !== undefined) projectToUpdate.code = newCode;
                if (newShift !== undefined)
                    projectToUpdate.shift = SHIFT[newShift].toString();
                if (newStandNumber !== undefined)
                    projectToUpdate.stand_number = newStandNumber;
                if (newIsEntrepreneurship !== undefined)
                    projectToUpdate.is_entrepreneurship = newIsEntrepreneurship;
                if (newResponsibles !== undefined) {
                    projectToUpdate.responsibles =
                        projectToUpdate.responsibles.concat(
                            newResponsibles.map((responsibleId) => {
                                const user = UserJson.userJson.find(
                                    (user) => user.user_id === responsibleId
                                );
                                if (user == null) {
