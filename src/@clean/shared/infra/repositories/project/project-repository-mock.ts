...
export class ProjectRepositoryMock implements IProjectRepository {
    ...
    async getProjectsByRole(userId: number): Promise<Project[]> {
        const json = ProjectJson.projectJson.filter(project => project.professors.some(professor => professor.user_id === userId) || project.students.some(student => student.user_id === userId));
