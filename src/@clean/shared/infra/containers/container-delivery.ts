...
import { CreateDeliveryUsecase } from "@modules/delivery/usecases/create-delivery-usecase.ts";
import { GetDeliveriesUsecase } from "@modules/delivery/usecases/get-deliveries-usecase.ts";
...
containerDelivery
    .bind(RegistryDelivery.DeliveryRepositoryMock)
    .to(DeliveryRepositoryMock);
containerDelivery
    .bind(RegistryDelivery.DeliveryRepositoryHttp)
    .toDynamicValue((context) => {
        return new DeliveryRepositoryHttp(
            context.container.get(RegistryDelivery.AxiosAdapter)
        );
    });

containerDelivery
    .bind(RegistryDelivery.CreateDeliveryUsecase)
    .toDynamicValue((context) => {
        if (import.meta.env.APP_STAGE === "TEST") {
            ...
            import.meta.env.APP_STAGE === "DEV" ||
            import.meta.env.APP_STAGE === "PROD"
        ) {
            return new CreateDeliveryUsecase(
...
containerDelivery
    .bind(RegistryDelivery.GetDeliveriesUsecase)
    .toDynamicValue((context) => {
        if (import.meta.env.APP_STAGE === "TEST") {
            return new GetDeliveriesUsecase(
                context.container.get(RegistryDelivery.DeliveryRepositoryMock)
            );
        } else if (
            import.meta.env.APP_STAGE === "DEV" ||
