...
containerTask
    .bind(RegistryTask.TaskRepositoryHttp)
    .toDynamicValue((context) => {
        return new TaskRepositoryHttp(
            context.container.get(RegistryTask.AxiosAdapter)
        );
    });

containerTask
    .bind(RegistryTask.GetAllTasksUsecase)
    .toDynamicValue((context) => {
        if (import.meta.env.APP_STAGE === "TEST") {
            return new GetAllTasksUsecase(
                context.container.get(RegistryTask.TaskRepositoryMock)
            );
        } else if (
            ...
            return new GetAllTasksUsecase(
                context.container.get(RegistryTask.TaskRepositoryHttp)
            );
        } else {
            return new GetAllTasksUsecase(
                context.container.get(RegistryTask.TaskRepositoryMock)
