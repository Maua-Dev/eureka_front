...
import {
    RegistryDelivery,
    containerDelivery
} from "@containers/container-delivery.ts";
...
export const DeliveryProvider = ({
    ...
    const [deliveriesFromContext, setDeliveriesFromContext] = useState<
        DeliveryModel[]
    >([]);
    ...
    const getDeliveries = async (projectId: number) => {
        ...
        const deliveriesModel = deliveriesCaught.map((delivery) =>
            DeliveryAdapter.toModel(delivery)
        );
    ...
    return (
        <DeliveryContext.Provider
            value={{ deliveriesFromContext, createDelivery, getDeliveries }}
        >
            {children}
        </DeliveryContext.Provider>
    );
