...
import {
    RegistryProject,
    containerProject
} from "@containers/container-project.ts";
import { CreateProjectUsecase } from "@modules/project/usecases/create-project-usecase.ts";
import { GetProjectUsecase } from "@modules/project/usecases/get-project-usecase.ts";
import { UpdateProjectUsecase } from "@modules/project/usecases/update-project-usecase.ts";
import { ProjectAdapter } from "@adapters/project-adapter";
import { GetProjectsByRoleUsecase } from "@modules/project/usecases/get-projects-by-role-usecase.ts";
...
export const ProjectProvider = ({
    ...
    const [projectsFromContext, setProjectsFromContext] = useState<
        ProjectModel[]
    >([]);
    const [projectFromContext, setProjectFromContext] = useState<ProjectModel>(
        ProjectModel.empty()
    );
    ...
    const createProject = async (projectModelToCreate: ProjectModel) => {
        ...
        const projectCreated =
            await createProjectUsecase.execute(projectToCreate);
    ...
    const getProjectsByRole = async (projectId: number) => {
        const projectsCaught =
            await getProjectsByRoleUsecase.execute(projectId);
        const projectsModel = projectsCaught.map((project) =>
            ProjectAdapter.toModel(project)
        );
