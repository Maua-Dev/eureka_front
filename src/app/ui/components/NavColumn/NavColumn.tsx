...
import { navOptionsList } from "@constants/nav-options-list.ts";
...
export default function NavColumn({ navColumnOptions }: NavColumnProps) {
    const [navStates, setNavStates] = useState<{ [key: string]: boolean }>(
        initialNavStates
    );
                ...
                ? navOptionsList.map((navOption, navOptionIndex) => {
                      return (
                          <DismissableDialog
                              key={navOptionIndex}
                              setOpen={() =>
                                  handleNavStates(navOption.stateKey!, false)
                              }
                          ...
                          >
                              ...
                              {navStates[navOption.stateKey!] && (
                                  <div className="nav_column--secondary">
                                      {navOption.secondaryOptions.map(
                                          (secondaryOption, index) => {
                                              return (
                                                  <Link
                                                      key={index}
