...
import { odsList } from "@constants/ods-list.ts";
import { actionsList } from "@constants/actions-list.ts";
...
export default function ProjectDataPage() {
    ...
    const { deliveriesFromContext, getDeliveries, createDelivery } =
    ...
    const [selectedOds, setSelectedOds] = useState<number[]>(
        (deliveriesFromContext.find(
            (delivery) => delivery.task.title === "Dados do trabalho"
                    ...
                    >
                        <div className="box__main">
                            ...
                            <DefaultTextField
                                ...
                                onSaveClick={() => {
                                    if (projectDescription === "") {
                                        toast.error(
                                            "A descrição do projeto não pode ser vazio"
                                        );
                                    } else if (
                                        projectDescription ===
                                        projectFromContext.description
                                        ...
                                        toast.error(
                                            "A descrição do projeto não pode ser igual ao anterior"
                                        );
                                    } else {
                                        handleFetch(
                                            setIsLoading,
                            ...
                            <span className="box__span">
                                ...
                                objetivos e 169 metas a serem atingidos até
                                2030. Para saber mais veja a tela de{" "}
                                <Link
                                    className="box__span"
                                    target="_blank"
                                    to={
                                        "https://sistema-eureka.maua.br/downloads/arquivos/ODS-agenda2030-pt-br.pdf"
                                    }
                                >
                                    downloads
                                </Link>{" "}
                            ...
                            {odsList.map((ods) => {
                                return (
                                    <ImageInfoCard
                                        ...
                                        key={ods.odsId}
                                        backgroundColor={
                                            selectedOds.includes(ods.odsId)
                                                ? "var(--dark-mustard)"
                                                : undefined
                                        }
                                        onClick={() => {
