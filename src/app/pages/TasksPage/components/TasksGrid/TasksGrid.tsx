...
import { TaskContentType } from "@@types/task-content-type.ts";
import { TaskModel } from "@models/task-model.ts";
import { DeliveryModel } from "@models/delivery-model.ts";
            ...
            <div className="tasks_grid">
                ...
                {tasksContent.map((taskContent, taskContentIndex) => {
                    const tasksFiltered = tasks.filter(
                        (task) => task.title === taskContent.title
                    );
                    ...
                    return (
                        <React.Fragment key={taskContentIndex}>
                            ...
                            >
                                <div className="tasks_grid__title">
                                    {taskContent.title}
                                </div>
                            </div>
                            <>
                                {tasksFiltered.map((task, taskIndex) => {
                                    return (
                                        <Link
                                            ...
                                            style={{
                                                gridColumn:
                                                    handleGridColumn(taskIndex),
                                                gridRow: `${taskContentIndex + 2} / ${taskContentIndex + 3}`
                                            }}
                                        >
                                            <p className="tasks_grid__text">
                                                Até {task.deliveryDate}
                                            </p>
                                            <p className="tasks_grid__text">
                                                Não enviado
                                            </p>
