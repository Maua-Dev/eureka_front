...
export default function FileUploadPage() {
    ...
    const [jobInfo, setJobInfo] = useState<TaskContentType | undefined>(
        jobInfoContentList.find(
            (jobInfo) =>
                jobInfo.title ===
                tasksFromContext.find((task) => task.taskId === taskIdFromPath)
                    ?.title
        )
    );
    ...
    useEffect(() => {
        setJobInfo(
            jobInfoContentList.find(
                (jobInfo) =>
                    jobInfo.title ===
                    tasksFromContext.find(
                        (task) => task.taskId === taskIdFromPath
                    )?.title
            )
        );
    }, [tasksFromContext]);
    ...
    const handleFileChange = (e: ChangeEvent<HTMLInputElement>) => {
        if (e.target.files) {
            Array.from(e.target.files).map((file) => {
                ...
                if (
                    jobInfo.maxSize &&
                    file.size > jobInfo.maxSize * 1024 ** 2
                ) {
                    toast.error(
                        `O tamanho do arquivo não pode exceder ${jobInfo.maxSize}MB`
                    );
                    return;
                }
                if (
                    jobInfo.maxNumber &&
                    files?.length &&
                    files.length >= jobInfo.maxNumber
                ) {
                    const fileText =
                        jobInfo.maxNumber > 1 ? "arquivos" : "arquivo";
                    toast.error(
                        `Você pode enviar no máximo ${jobInfo.maxNumber} ${fileText}`
                    );
                    return;
                }
