...
import { ResourceType } from "@@types/resource-type.ts";
...
export default function ResourceCard({
    ...
    const options: QuantityType[] = Array.from(
        { length: resource.maximum + 1 },
        (_, i) => ({
            quantity: i
        })
    );
                    ...
                    <div className="infos__quantity">
                        ...
                        <div className="quantity_input">
                            <ControlledTextField<QuantityType>
                                value={quantity}
                                options={options}
                                onChange={(value: QuantityType | null) =>
                                    setQuantity(value!)
                                }
                                getOptionLabel={(option: QuantityType) =>
                                    option.quantity.toString()
                                }
                                getOptionValue={(option: QuantityType) =>
                                    option.quantity.toString()
                                }
                                noOptionsMessage=""
